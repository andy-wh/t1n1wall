...
# mini httpd
	...
	tar -zxf $MW_BUILDPATH/freebsd8/build/local-sources/mini_httpd-1.22.tar.gz
	cd mini_httpd-1.22/
...
# ipfilter userland tools (newer version than included with FreeBSD)
	...
	make freebsd8
	install -s BSD/FreeBSD-8.*-$MW_ARCH/{ipf,ipfs,ipfstat,ipmon,ipnat,ippool} $MW_BUILDPATH/t1n1fs/sbin
# modem-stats
	cd $MW_BUILDPATH/tmp
...
# mbmon
	cd $PORTSDIR/sysutils/mbmon
        make
	install -s $WRKDIRPREFIX/$PORTSDIR/sysutils/mbmon/work/xmbmon*/mbmon $MW_BUILDPATH/t1n1fs/usr/local/bin/
# wol
	cd $PORTSDIR/net/wol
	make WITHOUT=NLS

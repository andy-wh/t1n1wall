...
# make filesystem structure for image
	mkdir  t1n1fs images
	cd t1n1fs
	mkdir -p etc/rc.d/ bin cf conf.default dev etc/mpd-modem ftmp mnt proc root sbin tmp libexec lib/casper /var/etc/dnsmasq usr/bin usr/lib usr/libexec usr/local usr/sbin usr/share usr/local/bin usr/local/captiveportal usr/local/lib/ipsec usr/local/sbin/.libs usr/local/www usr/share/misc boot/kernel
